...
const allQuotes = getAllPhilosophers().flatMap(p =>
  p.quotes.map(q => ({ ...q, author: p.name, authorSlug: p.slug }))
)
    ...
    <section className="relative bg-gradient-to-br from-aegean via-aegean to-aegean/90 text-ivory">
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32 text-center">
        <div className="inline-block mb-6 px-4 py-1.5 border border-gold/40 rounded-full">
          <span className="font-body text-gold text-xs tracking-[0.25em] uppercase">
            Bayt al-Ḥikma • Academia • Lyceum
          </span>
        </div>

        <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl text-gold tracking-wide leading-tight">
          Library of the Ancients
        </h1>
        <p className="font-body text-ivory/70 text-base sm:text-lg mt-4 max-w-2xl mx-auto leading-relaxed">
          A digital treasury of metaphysical wisdom — exploring the perennial philosophy
          through the works of the great thinkers who have illuminated the path of truth.
        </p>
      ...
      <div className="bg-aegean/80 backdrop-blur-sm border-t border-gold/20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 text-center">
          <p className="font-body text-gold/50 text-xs uppercase tracking-[0.2em] mb-3">
            Wisdom of the Ancients
          </p>
          {quote && (
            <div key={currentQuote} className="fade-in">
              <p className="font-body text-ivory/80 text-sm sm:text-base leading-relaxed italic">
                &ldquo;{quote.text}&rdquo;
              </p>
              <div className="mt-2">
                <Link
                  to={`/philosopher/${quote.authorSlug}`}
                  className="text-gold/60 hover:text-gold text-xs font-body transition-colors"
                >
                  &mdash; {quote.author}
                  {quote.source && <span>, <em>{quote.source}</em></span>}
                </Link>
              </div>
            </div>
          )}
