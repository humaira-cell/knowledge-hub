...
import { getBook, getPhilosopher } from '../lib/sanity'
import ComplexityBadge from '../components/ui/ComplexityBadge'

export default function BookDetail() {
  const { slug } = useParams()
  const { data: book, loading } = useAsync(() => getBook(slug), [slug])
  const { data: author } = useAsync(
    () => book?.authorId ? getPhilosopher(book.authorId) : Promise.resolve({ data: null }),
    [book?.authorId]
  )
  ...
  if (!book) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        ...
        <Link to="/catalog" className="text-turquoise hover:text-gold text-sm font-body mt-2 inline-block">Return to catalog</Link>
      </div>
    )
  }
      ...
      <div className="marble-bg border border-gold/30 rounded-lg overflow-hidden gold-glow">
        {book.coverUrl ? (
          <div className="h-48 sm:h-56 bg-gradient-to-br from-aegean/10 to-aegean/30 flex items-center justify-center overflow-hidden">
            <img src={book.coverUrl} alt={book.title} className="w-full h-full object-contain object-bottom opacity-90" />
          </div>
        ) : (
          <div className={`h-32 ${author?.portraitBg || 'from-aegean to-aegean'} bg-gradient-to-br flex items-center justify-center`}>
        ...
        <div className="p-6 lg:p-8">
          <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
            <h1 className="font-heading text-aegean text-2xl sm:text-3xl tracking-wide">{book.title}</h1>
            <ComplexityBadge level={book.complexity} />
          </div>

          {author && (
            <Link to={`/philosopher/${author.slug || author.id}`}
              className="inline-flex items-center gap-1.5 text-turquoise hover:text-gold text-sm font-body transition-colors mb-6">
              <User className="w-3.5 h-3.5" /> by {author.name}
            </Link>
          )}
          ...
          <div className="flex flex-wrap gap-4 mb-6 pb-6 border-b border-gold/20">
            ...
            <div className="text-center">
              <p className="font-heading text-aegean text-lg capitalize">{book.complexity}</p>
              <p className="font-body text-ink/40 text-[10px] uppercase tracking-wider">Level</p>
            </div>
            <div className="text-center">
              <p className="font-heading text-aegean text-lg capitalize">{book.era === 'ancient' ? 'Ancient' : 'Perennialist'}</p>
              <p className="font-body text-ink/40 text-[10px] uppercase tracking-wider">Era</p>
            </div>
          </div>

          <div>
            <h2 className="font-heading text-aegean text-lg tracking-wide mb-3">Metaphysical Description</h2>
            <p className="font-body text-ink/80 text-sm sm:text-base leading-relaxed">{book.description}</p>
          </div>
